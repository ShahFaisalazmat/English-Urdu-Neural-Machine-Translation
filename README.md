...
1. The decoder receives the previously generated token.
2. It predicts a probability distribution over the Urdu vocabulary.
3. The token with the highest probability is selected.
4. The generated token is fed back into the decoder.
5. Generation continues until `<EOS>` is produced or the maximum sequence length is reached.
...
| English Source | Reference Urdu | Model Output |
|---|---|---|
| Source sentence | Human translation | Generated translation |
